...
音量の正規化:
 - オーディオファイル毎に音量を調整し、同じ音量で聞こえるようにします

無音スキップ:
 - オーディオファイルの最後にある無音区間をスキップします
...
---
 - Q: 再生履歴は消去できないのですか
 - A: 30日経過すると自動で削除されます。また、「Settings」-「おすすめ」-「再生履歴を削除」からも削除できます。
---
 - Q: おすすめはどのようにして選ばれるのですか
 - A: 曲ごとの再生回数、アーティストごとの再生回数、タグごとの再生回数、追加した日時の4つの指標を用いて選出しています。
---
 - Q: おすすめが表示されません
 - A: 保存している曲が5曲以下の場合、あるいは30日間の再生回数が5回以下の場合は、おすすめを表示するためのデータが不足しているため表示されません。
---
 - Q: タグ機能が他の再生プレイヤーで確認できません
 - A: タグ機能は独自に追加したもので、曲データと共に`C:/Users/${username}/AppData/Roaming/kokone-music/track.json`に保存しているだけであるため、他のアプリではご使用いただけません。
---
...
---
 - Q: 音楽の追加後にその曲を再生しても音量が正規化されません
 - A: 音源をffmpegで解析し、音量を計算する部分で時間がかかってしまいます。再生中もバックグラウンドで解析を行い、解析後は次にその曲を再生し直したときに音量調整が適用されます。
---
...
---
 ...
 - A: 音声ファイルとその音声ファイルについての情報を保持しているjsonファイルで整合性が取れなくなっている可能性があります。直らない場合は一度曲をアプリ内から削除した後、再度「add music」のタブから追加してください。